...
{  	
   /* 当 SPI发送缓冲器非空时等待 */
  while (SPI_I2S_GetFlagStatus(NRF2_SPI, SPI_I2S_FLAG_TXE) == RESET);
  
   /* 通过 NRF2_SPI发送一字节数据 */
  SPI_I2S_SendData(NRF2_SPI, dat);		
 
   /* 当SPI接收缓冲器为空时等待 */
  while (SPI_I2S_GetFlagStatus(NRF2_SPI, SPI_I2S_FLAG_RXNE) == RESET);

  /* Return the byte read from the SPI bus */
  return SPI_I2S_ReceiveData(NRF2_SPI);
}
