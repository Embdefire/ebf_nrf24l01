...
{														 
	  ...
	  NRF1_CE_1();
	  CPU_TS_Tmr_Delay_US(10);                        //CE要拉高一段时间才进入发送模式
}
...
{
    GPIO_InitTypeDef GPIO_Initure;
	  RCC_PeriphCLKInitTypeDef SPI6ClkInit;
	
		//设置SPI2的时钟源 
		SPI6ClkInit.PeriphClockSelection=RCC_PERIPHCLK_SPI6;	    //设置SPI2时钟源
		SPI6ClkInit.Spi6ClockSelection=RCC_SPI6CLKSOURCE_PLL2;	//SPI2时钟源使用PLL1Q
		HAL_RCCEx_PeriphCLKConfig(&SPI6ClkInit);
	  /* 使能相应端口的时钟 */
    ...
    /* 初始化SPI的配置 */
    NRF2_Handler.Instance=NRF2_SPI;                      //SPI2
    ...
    __HAL_SPI_ENABLE(&NRF2_Handler);                 //使能SPI2
		NRF2_CE_0(); 			         
		/*置低CSN，使能SPI传输*/
		NRF2_CSN_1();			         
}
